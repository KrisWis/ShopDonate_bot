...
Bot = aiogram.Bot("5366109022:AAH_PzE6TcAUse11roxNQvgPmLgbFKLm2Us")
...
async def start(msg: Message):
    ...
    if msg.from_user.id not in admins:
        ...
        if cur.execute(f"SELECT user_id FROM users WHERE user_id={msg.from_user.id}").fetchone() is None:
            ...
            for i in admins:
                await Bot.send_message(admins[i], f'Вам пишет пользователь @{msg.from_user.username}! \nЧтобы начать разговор нажмите кнопку "Начать диалог ✅"', reply_markup=keyboard)
...
async def regular_message_handler(msg: Message):
    ...
    admin_data = []
    for i in admins:
        admin_data.append(cur.execute(f"SELECT * FROM users WHERE user_id={admins[i]}").fetchone())

    # if admin_data and admin_data[2] != msg.from_user.id and msg.from_user.id not in admins:
